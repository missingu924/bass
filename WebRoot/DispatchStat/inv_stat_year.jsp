...
<html>
	...
	%>
	<body>
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/DispatchStat/inv_stat_year.jsp">
		
			<table align="center" width="98%">
				<tr>
					<td align="center">
						<a href="<%=request.getContextPath()%>/DispatchStat/inv_stat_year.jsp?iyear=<%=iPreYear%>&invcode=<%=invcode%>"><input type="button" class="button button_left" title="上月" /> </a> &nbsp;
						<span style="color: #2281c8; font-size: 16px;"><%=inv.getCinvname()+" "+iyear+"年"%> 发货情况</span>&nbsp;
						<a href="<%=request.getContextPath()%>/DispatchStat/inv_stat_year.jsp?iyear=<%=iNextYear%>&invcode=<%=invcode%>"><input type="button" class="button button_right" title="下月" /> </a>
					</td>
				</tr>
				<tr>
			...
			<table id="overview_table" width="98%" align="center" class="table_goal">
				<tr onclick="openTab('产品 <%=invname+" "+iyear+"年"%> 发货明细','<%=request.getContextPath()%>/Proxy/Servlet?servlet=VDispatchLists&method=list4this&ddate_min=<%=startTimeYear%>&ddate_max=<%=endTimeYear%>&cinvcode=<%=invcode%>')">
					<td width="25%" height="120">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select count(distinct dl.DLID) value from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cinvcode='"+invcode+"'"), 0)%>
							</span>
						</p>
						<p>
							年度发货单数
					...
					<td width="25%">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select isnull(sum(iQuantity),0) value from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cinvcode='"+invcode+"'"), 0)%>
							</span>
						</p>
						<p>
							年度发货数量（<%=StringUtil.getNotEmptyStr(inv.getCcomunitname(),"无")%>）
						</p>
					</td>
					<td width="25%">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select isnull(sum(iNatSum)/sum(iQuantity),0) value from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cinvcode='"+invcode+"'"), 0)%>
							</span>
						</p>
						<p>
					...
					<td width="25%">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select isnull(sum(iNatSum)/10000,0) value from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cinvcode='"+invcode+"'"), 4)%>
							</span>
						</p>
						<p>
							年度发货金额（万元）
			...
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getRecentMonthSql4isum(iyear+"","12",12,invcode,null,null), "", "", new String[]{"发货金额","发货金额"}, new String[]{"月发货金额（万元）","去年同期发货金额（万元）"}, EchartsUtil.BAR, EchartsUtil.THEME_MACARONS, EchartsUtil.COLOR_BLUE, 300, 0, "openTab('产品 "+invname+" '+params.data.name+' 发货情况','"+request.getContextPath()+"/DispatchStat/inv_stat.jsp?invcode="+invcode+"&iyear='+params.data.name.substr(0,4)+'&imonth='+params.data.name.substr(5,6))")%>
				</td>
			</tr>
			</table>
			...
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getRecentMonthAvgPriceSql(iyear+"","12",12,invcode), "", "", new String[]{"平均单价"}, new String[]{"月度平均单价（元/"+inv.getCcomunitname()+"）"}, EchartsUtil.LINE, EchartsUtil.THEME_MACARONS, EchartsUtil.COLOR_ORANGE, 200, 0, null)%>
				</td>
			</tr>
			</table>
		...
		<script type="text/javascript">
			...
			{
				openTab('产品 <%=invname+" "+iyear+"年 " %>'+custname+' 发货明细','<%=request.getContextPath() %>/Proxy/Servlet?servlet=VDispatchLists&method=list4this&ddate_min=<%=startTimeYear %>&ddate_max=<%=endTimeYear %>&cinvcode=<%=invcode %>&ccuscode='+custcode);
			}
		</script>
	</body>
</html>
