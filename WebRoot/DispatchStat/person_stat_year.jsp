...
<html>
	...
	%>
	<body>
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/DispatchStat/person_stat_year.jsp">
		
			<table align="center" width="98%">
				<tr>
					<td align="center">
						<a href="<%=request.getContextPath()%>/DispatchStat/person_stat_year.jsp?iyear=<%=iPreYear%>&personcode=<%=personcode%>"><input type="button" class="button button_left" title="上月" /> </a> &nbsp;
						<span style="color: #2281c8; font-size: 16px;"><%=person.getCpersonname()+" "+iyear+"年"%> 发货情况</span>&nbsp;
						<a href="<%=request.getContextPath()%>/DispatchStat/person_stat_year.jsp?iyear=<%=iNextYear%>&personcode=<%=personcode%>"><input type="button" class="button button_right" title="下月" /> </a>
					</td>
				</tr>
				<tr>
			...
			<table id="overview_table" width="98%" align="center" class="table_goal">
				<tr onclick="openTab('业务员 <%=personname+" "+iyear+"年"%> 明细','<%=request.getContextPath()%>/Proxy/Servlet?servlet=VDispatchLists&method=list4this&ddate_min=<%=startTimeYear%>&ddate_max=<%=endTimeYear%>&cpersoncode=<%=personcode%>')">
					<td width="33%" height="120">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select count(*) value from DispatchList dl where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cpersoncode='"+personcode+"'"), 0)%>
							</span>
						</p>
						<p>
							年度发货单数
					...
					<td width="33%">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select isnull(sum(iQuantity),0) value from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cpersoncode='"+personcode+"'"), 0)%>
							</span>
						</p>
						<p>
							年度发货数量
					...
					<td width="33%">
						<p>
							<span class="lcd_blue_big">
							<%=StringUtil.formatDouble(EchartsUtil.getValueByInvSql("select isnull(sum(iNatSum)/10000,0) value from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID where dDate>='"+startTimeYear+"' and dDate<='"+endTimeYear+"' and cpersoncode='"+personcode+"'"), 4)%>
							</span>
						</p>
						<p>
							年度发货金额（万元）
		...
		<script type="text/javascript">
			...
			{
				openTab('业务员 <%=personname+" "+iyear+"年 " %>'+invname+' 发货明细','<%=request.getContextPath() %>/Proxy/Servlet?servlet=VDispatchLists&method=list4this&ddate_min=<%=startTimeYear %>&ddate_max=<%=endTimeYear %>&cpersoncode=<%=personcode %>&cinvcode='+invcode);
			}
			
			function dislist4cust(custcode,custname)
			{
				openTab('业务员 <%=personname+" "+iyear+"年 " %>'+custname+' 发货明细','<%=request.getContextPath() %>/Proxy/Servlet?servlet=VDispatchLists&method=list4this&ddate_min=<%=startTimeYear %>&ddate_max=<%=endTimeYear %>&cpersoncode=<%=personcode %>&ccuscode='+custcode);
			}
		</script>
	</body>
</html>
