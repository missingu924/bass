...
<html>
	...
	<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/DispatchStat/month_portal.jsp"> 
		<table align="center" width="98%">
			<tr>
				<td align="center">
					<a href="<%=request.getContextPath()%>/DispatchStat/month_portal.jsp?iyear=<%=iPreYear%>&imonth=<%=iPreMonth%>"><input type="button" class="button button_left" title="上月" /></a> 
					&nbsp;<span style="color:#000000;font-size:16px;font-weight:bold"><%=iyear%>年<%=imonth%>月发货概览</span>&nbsp;
					<a href="<%=request.getContextPath()%>/DispatchStat/month_portal.jsp?iyear=<%=iNextYear%>&imonth=<%=iNextMonth%>"><input type="button" class="button button_right" title="下月" /></a>
				</td>
			</tr>
		</table>
		...
		<table id="month_topn_table" align="center" width="98%">
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getTopnInventorySql(startTimeMonth,endTimeMonth,topn), "value desc", "", "产品", "月度"+topn+"大产品（万元）", EchartsUtil.BAR_V, EchartsUtil.THEME_SHINE, EchartsUtil.COLOR_PURPLE, 400, 300, "openTab('产品 '+params.data.name+' "+iyear+"/"+imonth+" 发货情况','"+request.getContextPath()+"/DispatchStat/inv_stat.jsp?iyear="+iyear+"&imonth="+imonth+"&invcode='+params.data.code)")%>
				</td>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getTopnCustomerSql(startTimeMonth,endTimeMonth,topn), "value desc", "", "客户", "月度"+topn+"大客户（万元）", EchartsUtil.BAR_V, EchartsUtil.THEME_SHINE, EchartsUtil.COLOR_GREEN, 400, 400, "openTab('客户 '+params.data.name+' "+iyear+"/"+imonth+" 发货情况','"+request.getContextPath()+"/DispatchStat/cust_stat.jsp?iyear="+iyear+"&imonth="+imonth+"&custcode='+params.data.code)")%>
				</td>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getTopnPersonSql(startTimeMonth,endTimeMonth,topn), "value desc", "", "业务员", "月度"+topn+"大业务员（万元）", EchartsUtil.BAR_V, EchartsUtil.THEME_SHINE, EchartsUtil.COLOR_CYAN, 400, 300, "openTab('业务员 '+params.data.name+' "+iyear+"/"+imonth+" 发货情况','"+request.getContextPath()+"/DispatchStat/person_stat.jsp?iyear="+iyear+"&imonth="+imonth+"&personcode='+params.data.code)")%>
				</td>
			</tr>
			<tr>
		...
		<table id="12month_table" align="center" width="98%">
			...
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getRecentMonthSql4invcount(iyear+"",imonth+"",recentMonths,null,null,null), "", "", new String[]{"发货产品种数","发货产品种数"}, new String[]{"发货产品种数","去年同期产品种数"}, EchartsUtil.LINE, EchartsUtil.THEME_MACARONS, EchartsUtil.COLOR_BLUE, 200, 0, "openTab(params.data.name+' 产品发货统计','"+request.getContextPath()+"/DispatchStat/inv_cust_stat.jsp?info_type=inv&iyear='+params.data.name.substr(0,4)+'&imonth='+params.data.name.substr(5,6))")%>
				</td>
			</tr>
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getRecentMonthSql4custcount(iyear+"",imonth+"",recentMonths,null,null,null), "", "", new String[]{"发货客户数","发货客户数"}, new String[]{"发货客户数","去年同期客户数"}, EchartsUtil.LINE, EchartsUtil.THEME_MACARONS, EchartsUtil.COLOR_BLUE, 200, 0, "openTab(params.data.name+' 客户发货统计','"+request.getContextPath()+"/DispatchStat/inv_cust_stat.jsp?info_type=cust&iyear='+params.data.name.substr(0,4)+'&imonth='+params.data.name.substr(5,6))")%>
				</td>
			</tr>
		</table>
	...
	<script type="text/javascript">
		...
		function invStat()
		{
			openTab('<%=iyear+"/"+imonth+""%> 产品分布','<%=request.getContextPath() %>/DispatchStat/inv_cust_stat.jsp?info_type=inv&iyear=<%=iyear%>&imonth=<%=imonth%>');
		}
		function custStat()
		{
			openTab('<%=iyear+"/"+imonth+""%> 客户分布','<%=request.getContextPath() %>/DispatchStat/inv_cust_stat.jsp?info_type=cust&iyear=<%=iyear%>&imonth=<%=imonth%>');
		}
		function personStat()
		{
