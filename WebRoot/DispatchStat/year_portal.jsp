...
<html>
	...
	<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/DispatchStat/year_portal.jsp"> 
		...
		<table id="5year_table" align="center" width="98%">
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getRecentYearSql(iYear+"", recentYears), "", "", new String[]{"发货额"}, new String[]{"近"+recentYears+"年发货额（万元）"}, EchartsUtil.BAR, EchartsUtil.THEME_ROMA, EchartsUtil.COLOR_BLUE, 300, 0, null)%>
				</td>
			</tr>
		</table>
		...
		<table id="year_topn_table" align="center" width="98%">
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getTopnInventorySql(startTimeYear,endTimeYear,topn), "value desc", "", "产品", "年度"+topn+"大产品（万元）", EchartsUtil.BAR_V, EchartsUtil.THEME_SHINE, EchartsUtil.COLOR_PURPLE, 400, 400, null)%>
				</td>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getTopnCustomerSql(startTimeYear,endTimeYear,topn), "value desc", "", "客户", "年度"+topn+"大客户（万元）", EchartsUtil.BAR_V, EchartsUtil.THEME_SHINE, EchartsUtil.COLOR_GREEN, 400, 400, null)%>
				</td>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(DispatchPortalUtil.getTopnPersonSql(startTimeYear,endTimeYear,topn), "value desc", "", "业务员", "年度"+topn+"大业务员（万元）", EchartsUtil.BAR_V, EchartsUtil.THEME_SHINE, EchartsUtil.COLOR_CYAN, 400, 300, null)%>
				</td>
			</tr>
			<tr>
				<td style="font-size:12px;color:#999999;text-align:center;vertical-align:top" height="40px">
					<a href="#" onclick="openTab('产品发货统计','<%=request.getContextPath() %>/Proxy/Servlet?servlet=VDispatchCustInvStat&method=list4this&groupBy=GROUP_BY_INV&ddate_min=<%=startTimeYear %>&ddate_max=<%=endTimeYear %>')">更多…</a>
				</td>
				<td style="font-size:12px;color:#999999;text-align:center;vertical-align:top">
					<a href="#" onclick="openTab('客户发货统计','<%=request.getContextPath() %>/Proxy/Servlet?servlet=VDispatchCustInvStat&method=list4this&groupBy=GROUP_BY_CUST&ddate_min=<%=startTimeYear %>&ddate_max=<%=endTimeYear %>')">更多…</a>
				</td>
				<td style="font-size:12px;color:#999999;text-align:center;vertical-align:top">
					<a href="#" onclick="">更多…</a>
				</td>
