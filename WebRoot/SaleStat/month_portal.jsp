...
<html>
	...
	<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/SaleStat/month_portal.jsp"> 
		...
		<table id="overview_table" width="98%" align="center" class="table_goal">
			<tr onclick="openTab('<%=iyear+"/"+imonth+" 销售分布"%>','<%=request.getContextPath()%>/SaleStat/inv_cust_stat.jsp?stat_type=month&iyear=<%=iyear%>&imonth=<%=imonth%>')">
		...
		<table id="12month_table" align="center" width="98%">
			...
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(SalePortalUtil.getRecentMonthSql4invcount(iyear+"",imonth+"",recentMonths,null,null,null), "", "", new String[]{"销售产品种数","销售产品种数"}, new String[]{"销售产品种数","去年同期产品种数"}, EchartsUtil.LINE, EchartsUtil.THEME_MACARONS, EchartsUtil.COLOR_GREEN_DARK, 200, 0, "openTab(params.data.name+' 产品销售统计','"+request.getContextPath()+"/SaleStat/inv_cust_stat.jsp?info_type=inv&iyear='+params.data.name.substr(0,4)+'&imonth='+params.data.name.substr(5,6))")%>
				</td>
			</tr>
			<tr>
				<td align="center">
					<%=EchartsUtil.createEchartByInvSql(SalePortalUtil.getRecentMonthSql4custcount(iyear+"",imonth+"",recentMonths,null,null,null), "", "", new String[]{"销售客户数","销售客户数"}, new String[]{"销售客户数","去年同期客户数"}, EchartsUtil.LINE, EchartsUtil.THEME_MACARONS, EchartsUtil.COLOR_GREEN_DARK, 200, 0, "openTab(params.data.name+' 客户销售统计','"+request.getContextPath()+"/SaleStat/inv_cust_stat.jsp?info_type=cust&iyear='+params.data.name.substr(0,4)+'&imonth='+params.data.name.substr(5,6))")%>
				</td>
			</tr>
		</table>
