...
<html>
	...
	<%
		...
		// 按产品统计
		String invStatSql = "select dls.cinvcode code,round(isnull(sum(iSum)/10000,0),4) value ,inv.cInvName name from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID left join inventory inv on dls.cInvCode=inv.cInvCode where dDate>='"
				+ (startTimeMonth) + "' and dDate<='" + (endTimeMonth) + "' group by dls.cinvcode,inv.cInvName";
		List invList = EchartsUtil.getInvListByInvSql(invStatSql, "value desc", "各产品销售额（万元）", "产品", "产品销售额（万元）");
		
		

		// 按客户统计
		String custStatSql = "select dl.cCusCode code,round(isnull(sum(iSum)/10000,0),4) value ,cust.cCusName name from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID left join customer cust on dl.cCusCode=cust.cCusCode where dDate>='"
				+ (startTimeMonth) + "' and dDate<='" + (endTimeMonth) + "' group by dl.cCusCode,cust.cCusName";
		List custList = EchartsUtil.getInvListByInvSql(custStatSql, "value desc", "各客户销售额（万元）", "客户", "客户销售额（万元）");
		
		// 按业务员统计
		String personStatSql = "select dl.cPersonCode code,round(isnull(sum(iSum)/10000,0),4) value ,isnull(per.cPersonName,'其他') name from DispatchList dl left join DispatchLists dls on dl.DLID=dls.DLID left join person per on dl.cPersonCode=per.cPersonCode where dDate>='"
				+ (startTimeMonth) + "' and dDate<='" + (endTimeMonth) + "' group by dl.cPersonCode,per.cPersonName";
		List personList = EchartsUtil.getInvListByInvSql(personStatSql, "value desc", "各业务员销售额（万元）", "业务员", "业务员销售额（万元）");
