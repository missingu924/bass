...
<html> 
	...
	<body> 
		...
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/<%=basePath%>/Servlet?method=import2db"> 
			<input type="hidden" id="dst_ds_code" name="dst_ds_code" value="<%=domainInstance.getDst_ds_code() %>">
			<input type="hidden" id="src_ds_code" name="src_ds_code" value="<%=domainInstance.getSrc_ds_code() %>">
			<!-- 数据处理方式 --> 
			<table class="search_table" align="center" width="98%"> 
				<tr> 
					...
					<td align="left" style="width:200px"> 
						请选择导入方式
						<%=DictionaryUtil.getSelectHtml(new DictionaryService().getDictItemsByDictName("导入数据处理方式字典", true), "import_type", "", StringUtil.getNotEmptyStr(domainInstance.getImport_type(), ""), "notEmpty")%> 
						&nbsp;  
				...
				<% 
					...
					{ 
							...
							for(int n=0;n<columnEnNamesList.size();n++)
							{
								...
								PropertyDescriptor p = PropertyUtils.getPropertyDescriptor(o,columnEnName);
								String pValue = BeanUtils.getProperty(o,columnEnName);
								if(p.getPropertyType().equals(Double.class))
								{
									out.print("<td>"+(StringUtil.isEmpty(pValue)?"":StringUtil.formatDouble(Double.parseDouble(pValue),4))+"</td>");
								}
								else
								{
									out.print("<td>"+StringUtil.getNotEmptyStr(pValue)+"</td>");
								}
							} 
		...
		<table width="98%" align="center" class="title_table"> 
			<tr> 
				<td> 
					<img src="../images/excel.png" width="18" height="18" align="absmiddle"> 
					&nbsp;&nbsp;共取出<font color="#ff9900"><%=excelDataList.size()%></font>条数据<span id="cover_info" style="display:none;color:#ff9900">，将删除系统中与这批数据重复的数据并导入这批新数据</span>
	...
	<script>
	 ...
	 $('#import_type').change(function(){
	 	...
	 	else if('覆盖'==selected)
	 	{
	 		...
	 		all_data_div.show();
	 		cover_info.show();
	 		same_record_process_type_div.hide();
	 	}
	 	else if(''==selected)
	 	{
	 		new_data_div.hide();
