...
public class DictionaryUtil
{
	...
	{
		return getInputHtml(dictName, inputName, inputValue, null, null, dbFilter, size, multiSelect);
	}
	...
	{
		return getInputHtml(dictName, inputName, inputValue, null, null, dbFilter, 40, multiSelect);
	}
	 ...
	 * @return
	 */
	public static String getInputHtml(String dictName, String inputName, String inputValue, String parentDictName, String parentDictInputName, String dbFilter, int size, boolean multiSelect)
	{
		StringBuffer sb = new StringBuffer();
		...
		sb.append("<input type=\"text\" id=\"" + inputName4show + "\" name=\"" + inputName4show + "\" value=\"" + getValueByKey(dictName, inputValue) + "\" onclick=\"selectFromDictionary('" + dictName + "','" + inputName + "','" + inputName4show + "',"
				+ (!StringUtil.isEmpty(parentDictName) ? ("'" + parentDictName + "',$('#" + parentDictInputName) + "').val()," : "'','',") + (!StringUtil.isEmpty(dbFilter) ? ("'" + dbFilter + "'") : "''") + ",multiSelect=" + multiSelect + ")\" readOnly size=\"" + size + "\"><input id=\""
	...
	{
		return getInputHtml(dictName, inputName, inputValue, parentDictName, parentDictInputName, null, size, multiSelect);
	}
